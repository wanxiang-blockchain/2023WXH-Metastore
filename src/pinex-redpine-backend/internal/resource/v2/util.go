...
func getRandomIps(params *commonParams, number int) (string, string, []string, error) {
	...
	credential.PrivateKey = params.token.AccessKeySecret

	client := vpc.NewClient(&cfg, &credential)
	describeSubnetReq := client.NewDescribeSubnetRequest()
	describeSubnetReq.Zone = ucloud.String(params.az)
	...
	describeSubnetResourceReq.Zone = ucloud.String(params.az)
	...
	for ip := ip.Mask(ipnet.Mask); ipnet.Contains(ip); inc(ip) {
		...
		// get 100 candidates for only one ip
		if len(ips) == number*50 {
			break
		}
	}
	...
	// select random ips from ip candidates
	rand.Shuffle(len(ips), func(i, j int) {
		ips[i], ips[j] = ips[j], ips[i]
	})
	return vpcId, subnetId, ips[:number], nil
}
...
func prettyRoleString(s string) string {
